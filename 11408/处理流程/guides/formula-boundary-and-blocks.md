...
```bash
node 11408/处理流程/scripts/_validate_blocks.cjs <md>        # 跨行 $$ 块 KaTeX 校验（补盲区，最关键）
python 11408/处理流程/scripts/_scan_dollar_balance2.py       # 公式边界解析（跳过 \$ 转义、块内 $ 不算边界）→ 应"边界完整"
python 11408/处理流程/scripts/_scan_dollar_lines.py          # 逐行单 $ 配对（跳过代码块/$$/\$）→ 应无奇数行
python 11408/处理流程/scripts/_scan_dollar_unmatched.py      # 定位孤立 $/$$（承受点，修完重跑）
python 11408/处理流程/scripts/_scan_unknown_cmds.py          # 未知 LaTeX 命令（OCR 截断）
```
...
**关键检测工具（补所有盲区）**：
```bash
node 11408/处理流程/scripts/_validate_all.cjs <md>   # 全文 KaTeX 校验：状态机提取所有 $...$ 和 $$...$$（含跨行行内），逐个渲染。终极权威！
python 11408/处理流程/scripts/_show_inline_bb.py <md>   # 定位行内嵌 $$
python 11408/处理流程/scripts/_scan_crossblock.py <md>  # 定位跨行 $...$ 块（$ + \left{/\begin{）
```
