...
【解析】方法一$f_{y}^{\prime}(x,y)=xe^{x^{3}y^{2}}$,$f_{y}^{\prime}(x,1)=xe^{x^{3}}$;$f_{yx}^{\prime\prime}(x,1)=3x^{3}e^{x^{3}}+e^{x^{3}}$,$f_{yx}^{\prime\prime}(1,1)=4e$.

由于$f(x,y)$的二阶混合偏导数在点(1,1)处是相等的，因此$f_{xy}^{\prime\prime}(1,1)=4e$

方法二 当x>0时，$f(x,y)=\int_{0}^{xy}e^{xt^{2}}dt\xrightarrow{\frac{u}{\sqrt{x}}=t}\int_{0}^{x^{\frac{3}{2}}y}e^{u^{2}}\frac{1}{\sqrt{x}}du=\frac{1}{\sqrt{x}}\int_{0}^{x^{\frac{3}{2}}y}e^{u^{2}}du$，得
...
故当x=1时，有$f_{x}^{\prime}(1,y)=-\frac{1}{2}\int_{0}^{y}e^{u^{2}}du+\frac{3}{2}e^{y^{2}}\cdot y$，则

$$f_{x y}^{\prime\prime}(1,y)=-\frac{1}{2}\mathrm{e}^{y^{2}}+\frac{3}{2}\cdot\mathrm{e}^{y^{2}}\cdot2y\cdot y+\frac{3}{2}\mathrm{e}^{y^{2}}\cdot1,$$

$$f_{xy}^{\prime \prime}(1,1)=-\frac{1}{2}\mathbf{e}+3\mathbf{e}+\frac{3}{2}\mathbf{e}=4\mathbf{e}.$$
...
【解析】在$z+\ln z-\int_{y}^{x}e^{-t^{2}}dt=1$两边对x求偏导，有
...
$$\begin{array}{r l}{\frac{\partial^{2}z}{\partial x\partial y}}&{=\mathbf{e}^{-x^{2}}\ \frac{\partial}{\partial y}\left(\frac{z}{1+z}\right)=\mathbf{e}^{-x^{2}}\ \frac{\frac{\partial z}{\partial y}}{\left(1+z\right)^{2}}=-\frac{z}{\left(1+z\right)^{3}}\ \mathbf{e}^{-\left(x^{2}+y^{2}\right)}\ .}\end{array}$$

将x=0，y=0代入$z+\ln z-\int_{y}^{x}e^{-t^{2}}dt=1$，得$z+\ln z-1=0$。令$f(z)=z+\ln z-1$，则

$$\lim_{z\to0^{+}}f(z)=-\infty,\quad\lim_{z\to+\infty}f(z)=+\infty,f^{\prime}(z)=1+\frac{1}{z}>0,$$

所以$f(z)$有唯一零点.易见$f(1)=0$,所以x=0,y=0时,z=1,代入(*)式,得$\left.\frac{\partial^{2}z}{\partial x\partial y}\right|_{(0,0)}=-\frac{1}{8}$
#### 16.
...
<img src="../../../../../origin/27张宇1000题数一【解析册】/imgs/img_63.jpg">
...
解得y=0，$4x+8z=0$。再与$2x^{2}+2y^{2}+z^{2}+8xz-z+8=0$联立解得两组解：
...
$$\left.\frac{\partial^{2}z}{\partial x^{2}}\right|_{\left(\frac{16}{7},0,-\frac{8}{7}\right)}=-\left.\frac{4}{15}\right.,\left.\frac{\partial^{2}z}{\partial x\partial y}\right|_{\left(\frac{16}{7},0,-\frac{8}{7}\right)}=0,\left.\frac{\partial^{2}z}{\partial y^{2}}\right|_{\left(\frac{16}{7},0,-\frac{8}{7}\right)}=-\left.\frac{4}{15}\right..$$
...
【解析】将方程$x^2 - xy + y^2 = 1$两边对x求导，得$2x - y - xy' + 2y \cdot y' = 0$，则$y' = \frac{y - 2x}{2y - x}$，即切线方程为$Y - y = \frac{y - 2x}{2y - x}(X - x)$。

令 Y=0，则其在 x 轴上的截距为$X=x-\frac{y(2y-x)}{y-2x}$；令 X=0，则其在 y 轴上的截距为 Y=y-$\frac{x(y-2x)}{2y-x}$. 故所求面积为

$$\begin{aligned}S=&\frac{1}{2}\;XY=\frac{1}{2}\bullet\frac{xy-2x^{2}-2y^{2}+xy}{y-2x}\bullet\frac{2y^{2}-xy-xy+2x^{2}}{2y-x}\\=&\frac{1}{2}\bullet2\bullet2\frac{\left\lbrack xy-(x^{2}+y^{2})\right\rbrack(x^{2}+y^{2}-xy)}{(y-2x)(2y-x)}.\end{aligned}$$
...
$$\left\{\begin{aligned}F_{x}^{\prime}=&5y-4x+\lambda(2x-y)=0,\\ F_{y}^{\prime}=&5x-4y+\lambda(2y-x)=0,\\ F_{\lambda}^{\prime}=&x^{2}-xy+y^{2}-1=0,\end{aligned}\right.$$

解得x=y=1，故$S_{\min}=\frac{2}{\left|(-1)\cdot(-1)\right|}=2$，即点P坐标为$(1,1)$.
...
<img src="../../../../../origin/27张宇1000题数一【解析册】/imgs/img_64.jpg">
