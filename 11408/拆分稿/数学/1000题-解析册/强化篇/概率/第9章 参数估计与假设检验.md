...
$$F_{(n)}(t)=\left[F_{X}(t)\right]^{n}=\left\{\begin{aligned}&0,&&t<0,\\ &\frac{t^{n}}{\theta^{n}},&&0\leq t<\theta,\\ &1,&&t\geq\theta,\end{aligned}\right.$$

于是$Z = \frac{\hat{\theta}}{\theta}$的分布函数为

$$\begin{aligned}F_{Z}(z)&=P\left\{Z\leq z\right\}=P\left\{\frac{\hat{\theta}}{\theta}\leq z\right\}=P\left\{\frac{X_{(n)}}{\theta}\leq z\right\}\\&=P\left\{X_{(n)}\leq\theta z\right\}=F_{(n)}(\theta z)=\left\{\begin{aligned}&0,&\theta z<0,\\&\frac{(\theta z)^{n}}{\theta^{n}},&0\leq\theta z<\theta,\\&1,&\theta z\geq\theta\end{aligned}\right.\\&=\left\{\begin{aligned}&0,&z<0,\\&z^{n},&0\leq z<1,\\&1,&z\geq1.\end{aligned}\right.\\ \end{aligned}$$

(3)

$$P\left\{\hat{\theta}<\theta<\theta_{0}\right\}=P\left\{\frac{\hat{\theta}}{\theta_{0}}<\frac{\hat{\theta}}{\theta}<\frac{\hat{\theta}}{\hat{\theta}}\right\}=P\left\{\frac{\hat{\theta}}{\theta_{0}}<\frac{\hat{\theta}}{\theta}<1\right\}=1-\alpha,$$

即

$$F_{Z}(1)-F_{Z}\left(\frac{\hat{\theta}}{\theta_{0}}\right)=1-\frac{(\hat{\theta})^{n}}{\theta_{0}^{n}}=1-\alpha,$$

整理得$\frac{(\hat{\theta})^{n}}{\theta_{0}^{n}}=\alpha$，解得$\theta_{0}=\sqrt[n]{\frac{(\hat{\theta})^{n}}{\alpha}}=\frac{\hat{\theta}}{\sqrt[n]{\alpha}}=\frac{X_{(n)}}{\sqrt[n]{\alpha}}$
...
令$\mu_{1}=\bar{X}$，即$\frac{\sqrt{2\theta\pi}}{2}=\frac{1}{n}\sum_{i=1}^{n}X_{i}$，解得$\theta$的矩估计量为$\hat{\theta}_{M}=\frac{2}{\pi}\bar{X}^{2}$.

$$\begin{aligned}E(X^{2})=&\int_{0}^{+\infty}x^{2}\cdot\frac{x}{\theta}\mathrm{e}^{-\frac{x^{2}}{2\theta}}\mathrm{d}x=-\int_{0}^{+\infty}x^{2}\mathrm{d}\left(\mathrm{e}^{-\frac{x^{2}}{2\theta}}\right)\\=&-x^{2}\mathrm{e}^{-\frac{x^{2}}{2\theta}}\Bigg|_{0}^{+\infty}-2\theta\int_{0}^{+\infty}\mathrm{e}^{-\frac{x^{2}}{2\theta}}\mathrm{d}\left(\frac{-x^{2}}{2\theta}\right)\\=&-2\theta\mathrm{e}^{-\frac{x^{2}}{2\theta}}\Bigg|_{0}^{+\infty}=2\theta,\end{aligned}$$

故

$$\begin{aligned}E(\hat{\theta}_{M})=&\frac{2}{\pi}E(\overline{X}^{2})=\frac{2}{\pi}\left\{D(\overline{X})+\left[E(\overline{X})\right]^{2}\right\}=\frac{2}{\pi}\left\{\frac{1}{n}D(X)+\left[E(X)\right]^{2}\right\}\\=&\frac{2}{\pi}\left\{\frac{1}{n}E(X^{2})-\frac{1}{n}\left[E(X)\right]^{2}+\left[E(X)\right]^{2}\right\}\\=&\frac{2}{\pi}\left(\frac{2\theta}{n}-\frac{\theta\pi}{2n}+\frac{\theta\pi}{2}\right).\end{aligned}$$

设$x_{1}, x_{2}, \cdots, x_{n}$为样本值，则似然函数为

$$L(\theta)=\left\{\begin{aligned}&\prod_{\substack{i=1\\}}^{n}x_{i}\quad\sum_{\substack{i=1\\}}^{n}x_{i}^{2}\\&\frac{\theta^{n}}{\quad}\quad\mathrm{e}^{\frac{\quad}{\quad}}\frac{2\theta}{\quad},\quad x_{i}>0(i=1,2,\cdots,n),\\&0,\quad\quad\quad 其他 ,\end{aligned}\right.$$

当$x_i > 0 (i=1,2,\cdots,n)$时，取对数得$\ln L(\theta) = \ln \prod_{i=1}^{n} x_i - n \ln \theta - \frac{\sum_{i=1}^{n} x_i^2}{2\theta}$，令

$$\frac{\mathrm{d}\left[\ln L(\theta)\right]}{\mathrm{d}\theta}=-\frac{n}{\theta}+\frac{\sum_{i=1}^{n}x_{i}^{2}}{2\theta^{2}}=0,$$

解得$\theta=\frac{\displaystyle\sum_{i=1}^{n}x_{i}^{2}}{2n}$，故$\theta$的最大似然估计量为$\hat{\theta}_{L}=\frac{\displaystyle\sum_{i=1}^{n}X_{i}^{2}}{2n}$.

$$E(\hat{\theta}_{L})=\frac{1}{2n}\;E\left(\sum_{i=1}^{n}X_{i}^{2}\right)=\frac{1}{2}\;E(X^{2})=\frac{1}{2}\bullet2\theta=\theta.$$
...
【解析】(1)设$x_{1}, x_{2}, \cdots, x_{n}$为简单随机样本的样本值，则似然函数为
...
当$x_{i}\geq\beta(i=1,2,\cdots,n)$时，

$$\ln L=n\ln\alpha+n\alpha\ln\beta-(\alpha+1)\sum_{i=1}^{n}\ln x_{i},$$

由于$\frac{\partial(\ln L)}{\partial\beta}=\frac{n\alpha}{\beta}>0$，因此$\beta$的最大似然估计量为$\hat{\beta}=X_{(1)}=\min_{1\leq i\leq n}\{X_i\}$.

又$\frac{\partial(\ln L)}{\partial\alpha}=\frac{n}{\alpha}+n\ln\beta-\sum_{i=1}^{n}\ln x_{i}\xrightarrow{\text{令}}0$，则

$$\alpha=\frac{1}{n\sum_{i=1}^{n}\ln x_{i}-\ln\beta}=\frac{1}{\frac{1}{n}\sum_{i=1}^{n}\ln\frac{x_{i}}{\beta}},$$

故$\hat{\alpha} = \frac{1}{\frac{1}{n} \sum_{i=1}^{n} \ln \frac{X_{i}}{X_{(1)}}}$

(2)由题意知，X的分布函数为

$$F(x)=\int_{-\infty}^{x}f(t;\alpha,\beta)\mathrm{d}t=\left\{\begin{aligned}&1-\left(\frac{\beta}{x}\right)^{\alpha},&x\geq\beta,\\ &0,& 其他 ,\end{aligned}\right.$$

故

$$X_{(1)}\sim F_{(1)}(x)=1-\left[1-F(x)\right]^{n}=\left\{\begin{aligned}&1-\left(\frac{\beta}{x}\right)^{n\alpha},&x\geq\beta,\\ &0,& 其他 .\end{aligned}\right.$$

$$\begin{align*}P\left\{\left|\hat{\beta}-a\right|\geqslant\varepsilon\right\}&=P\left\{\hat{\beta}\geqslant a+\varepsilon\right\}+P\left\{\hat{\beta}\leqslant a-\varepsilon\right\}\\&=P\left\{X_{(1)}\geqslant a+\varepsilon\right\}+P\left\{X_{(1)}\leqslant a-\varepsilon\right\}\\&=1-P\left\{X_{(1)}<a+\varepsilon\right\}+P\left\{X_{(1)}\leqslant a-\varepsilon\right\},\end{align*}$$

当$a>\beta$或$a<\beta$时，均存在$\varepsilon>0$，使$\lim_{n\to\infty}P\left\{\left|\hat{\beta}-a\right|\geq\varepsilon\right\}=1$，不符题意.

当$a = \beta$时，$P\left\{\left|\hat{\beta} - a\right| \geq \varepsilon\right\} = 1 - \left[1 - \left(\frac{\beta}{\beta + \varepsilon}\right)^{na}\right] + 0$，则$\lim_{n \to \infty} P\left\{\left|\hat{\beta} - a\right| \geq \varepsilon\right\} = 1 - 1 = 0$。

(3)$E(\ln X_1) = \int_{\beta}^{+\infty} \ln x \cdot \frac{\alpha\beta^{\alpha}}{x^{\alpha+1}} \, \mathrm{d}x = \alpha\beta^{\alpha} \int_{\beta}^{+\infty} \ln x \, \mathrm{d}\left(-\frac{1}{\alpha} x^{-\alpha}\right) = \ln\beta + \frac{1}{\alpha}$.
...
【解析】设X的数学期望为$\mu$.尽管未说明X是否为正态总体，但是由中心极限定理，易得

$$Z=\frac{\overline{X}-\mu}{1/\sqrt{16}}\sim N(0,1).$$

又由$\Phi(1.96)=0.975$，故$P\left\{-1.96<Z<1.96\right\}=0.95$，即$\mu$的置信度等于 0.95 的置信区间为$\left(\overline{X}-\frac{1}{4}\times1.96,\overline{X}+\frac{1}{4}\times1.96\right)$，将$\overline{X}=a$代入上式得置信区间为$(a-0.49,\ a+0.49)$。
...
【解析】由题设，$H_{0}:X\sim N(0,1)$，$H_{1}:X\sim N(1,1)$，故$\beta=P\left\{\left|\overline{W}\right|H_{1}\right\}=P\left\{\left|\overline{X}<0.55\right|H_{1}\right\}$.

在$H_1$成立的条件下，$\overline{X} \sim N\left(1, \frac{1}{9}\right)$，故$\beta = P\left\{\frac{\overline{X} - 1}{\frac{1}{3}} < \frac{0.55 - 1}{\frac{1}{3}}\right\} = \Phi(-1.35) = 1 - \Phi(1.35)$.
...
<img src="../../../../../origin/27张宇1000题数一【解析册】/imgs/img_124.jpg">
