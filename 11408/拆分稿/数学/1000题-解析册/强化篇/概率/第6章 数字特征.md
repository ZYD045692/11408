...
| X | 0 | 1 |  | Y | 0 | 1 |
|---|---|---|---|---|---|---|
| P | $\frac{3}{4}$ | $\frac{1}{4}$ | ; Y~B$(1,\frac{1}{6})$, 即 | P | $\frac{5}{6}$ | $\frac{1}{6}$ |

已知$\mathrm{Cov}(X,Y)$，由$\mathrm{Cov}(X,Y)=E(XY)-E(X)\cdot E(Y)$，就可以求出$E(XY)$，而 XY 的取值也一定为 0, 1,$E(XY)=1\cdot P\{XY=1\}+0\cdot P\{XY=0\}=P\{XY=1\}$。从而得出$(X,Y)$的概率分布，其余值就不难算出。
...
| $p_{\bullet j}$ | $\frac{5}{6}$ | $\frac{1}{6}$ | 1 |

(2)由于$X\sim B\left(1,\frac{1}{4}\right)$，因此$D(X)=npq=\frac{1}{4}\times\frac{3}{4}=\frac{3}{16}$，同理$D(Y)=\frac{1}{6}\times\frac{5}{6}=\frac{5}{36}$，

$$\rho_{XY}=\frac{\mathrm{Cov}(X,Y)}{\sqrt{D(X)}\sqrt{D(Y)}}=\frac{\frac{1}{24}}{\frac{\sqrt{15}}{24}}=\frac{\sqrt{15}}{15}.$$

(3)因为XY取值只有0和1，所以$P\{XY=0\}=1-P\{XY=1\}=1-\frac{1}{12}=\frac{11}{12}$.

#### 12.

【解析】记Y为利润，则$Y=g(X)=\left\{\begin{aligned}&(1000-300-400)\cdot m,&X>m,\\&(1000-300)X-400m,&X\leq m,\end{aligned}\right.$又

$$X\sim f(x)=\left\{\begin{aligned}&\frac{1}{1000}\mathrm{e}^{-\frac{x}{1000}},&x\geqslant0,\\ &0,&x<0,\end{aligned}\right.$$

故

$$E(Y)=\int_{-\infty}^{+\infty}g(x)f(x)\mathrm{d}x=\int_{0}^{m}(700x-400m)\frac{\mathrm{e}^{-\frac{x}{1000}}}{1000}\mathrm{d}x+\int_{m}^{+\infty}300m\cdot\frac{\mathrm{e}^{-\frac{x}{1000}}}{1000}\mathrm{d}x$$

$$\begin{aligned}\frac{\mathrm{d}[E(Y)]}{\mathrm{d}m}=&-\frac{2}{5}\int_{0}^{m}\mathrm{e}^{\frac{x}{1000}}\mathrm{d}x+\frac{3}{10}\int_{m}^{+\infty}\mathrm{e}^{-\frac{x}{1000}}\mathrm{d}x=400\bullet\mathrm{e}^{\frac{x}{1000}-\frac{x}{1000}}\Bigg|_{0}^{m}-300\bullet\mathrm{e}^{-\frac{x}{1000}}\Bigg|_{m}^{+\infty}\\=&400\mathrm{e}^{\frac{m}{1000}}-400+300\mathrm{e}^{\frac{m}{1000}}=700\mathrm{e}^{\frac{m}{1000}}-400=0.\end{aligned}$$

得$e^{\frac{m}{1000}} = \frac{4}{7}$，故当$m = -1000 \ln \frac{4}{7} \approx 560$时，期望利润最大.
...
【解析】(1)由题意得，当0<x<1时，$f_{Y\mid X}(y\mid x)=\left\{\begin{aligned}&\frac{1}{2x},&-x<y<x,\\&0,& 其他,\end{aligned}\right.$则
...
$$f_{Y}(y)=\int_{-\infty}^{+\infty}f(x,y)\mathrm{d}x=\left\{\begin{aligned}&1-\left|y\right|,&-1<y<1,\\ &0,& 其他 .\end{aligned}\right.$$

在$Y = y (-1 < y < 1)$的条件下，X 的条件概率密度为

$$f_{X\mid Y}(x\mid y)=\frac{f(x,y)}{f_{Y}(y)}=\left\{\begin{aligned}&\frac{1}{1-\left|y\right|},&\left|y\right|<x<1,\\ &0,& 其他 .\end{aligned}\right.$$

所以$P\left\{\left.\frac{1}{2}<X<\frac{3}{2}\right|Y=0\right\}=\int_{\frac{1}{2}}^{\frac{3}{2}}f_{X\mid Y}(x\mid y=0)dx=\int_{\frac{1}{2}}^{1}1dx=\frac{1}{2}$.

(2) 因为$f(x, y) \neq f_{X}(x) f_{Y}(y)$，所以 X 与 Y 不独立.

由$E(XY)=\int_{-\infty}^{+\infty}\int_{-\infty}^{+\infty}xyf(x,y)dxdy=\int_{0}^{1}x\mathrm{d}x\int_{-x}^{x}y\mathrm{d}y=0$，$E(Y)=0$，得

$$\operatorname{C o v}(X,Y)=E(X Y)-E(X)E(Y)=0,$$

故X与Y不相关.

(3) 因 Z = X - Y，则当$z \leq 0$时，$f_{Z}(z) = 0$；当 0 < z < 2 时，
...
$$f_{Z}(z)=\left\{\begin{aligned}&1-\frac{z}{2}&,0<&z<2,\\ &0&, 其他 .\end{aligned}\right.$$
...
$$f_{X}(x)=\left\{\begin{aligned}&1,&0<x<1,\\ &0,& 其他 ,\end{aligned}\right.$$
...
$$f(x,y)=f_{X}(x)f_{Y\mid X}(y\mid x)=\left\{\begin{aligned}&\frac{1}{x},&0<y<x<1,\\ &0,& 其他 ,\end{aligned}\right.$$

Y的概率密度为

$$f_{Y}(y)=\int_{-\infty}^{+\infty}f(x,y)\mathrm{d}x=\left\{\begin{aligned}&-\ln y,&0<y<1,\\ &0,& 其他 .\end{aligned}\right.$$
...
$$E(XY)=\int_{-\infty}^{+\infty}\int_{-\infty}^{+\infty}xy\cdot f(x,y)dxdy=\int_{0}^{1}dx\int_{0}^{x}ydy=\frac{1}{6}$$
...
$$D(Y)=E(Y^{2})-\left[E(Y)\right]^{2}=\frac{7}{144},$$

因此

$$\rho_{XY}=\frac{\mathrm{Cov}(X,Y)}{\sqrt{D(X)}\sqrt{D(Y)}}=\frac{E(XY)-E(X)E(Y)}{\sqrt{D(X)}\sqrt{D(Y)}}=\frac{\frac{1}{6}-\frac{1}{2}\times\frac{1}{4}}{\sqrt{\frac{1}{12}}\times\sqrt{\frac{7}{144}}}=\frac{\sqrt{21}}{7}\;.$$
...
$$\begin{aligned}D(Y)=&E\left[D(Y\left|X\right)\right]+D\left[E(Y\left|X\right)\right]=E\left(\frac{X^{2}}{12}\right)+D\left(\frac{X}{2}\right)\\=&\frac{1}{12}E(X^{2})+\frac{1}{4}D(X)=\frac{1}{12}\int_{0}^{1}x^{2}\mathrm{d}x+\frac{1}{4}\bullet\frac{1}{12}=\frac{7}{144},\end{aligned}$$

$$E(XY)=E\left[E(XY\mid X)\right]=E\left(\frac{X^{2}}{2}\right)=\frac{1}{2}E(X^{2})=\frac{1}{6},$$

因此

$$\rho_{XY}=\frac{\mathrm{Cov}(X,Y)}{\sqrt{D(X)}\sqrt{D(Y)}}=\frac{\frac{1}{6}-\frac{1}{2}\times\frac{1}{4}}{\sqrt{\frac{1}{12}}\times\sqrt{\frac{7}{144}}}=\frac{\sqrt{21}}{7}\;.$$
...
<img src="../../../../../origin/27张宇1000题数一【解析册】/imgs/img_123.jpg">
