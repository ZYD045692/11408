...
$$\begin{array}{c}\displaystyle\int\mathrm{e}^{2x}\arctan\sqrt{\mathrm{e}^{x}-1}\mathrm{d}x=\frac{1}{2}\int\arctan\sqrt{\mathrm{e}^{x}-1}\mathrm{d}(\mathrm{e}^{2x})\\ \\ =\frac{1}{2}\mathrm{e}^{2x}\arctan\sqrt{\mathrm{e}^{x}-1}-\frac{1}{4}\int\frac{\mathrm{e}^{2x}}{\sqrt{\mathrm{e}^{x}-1}}\mathrm{d}x\;.\end{array}$$

又

$$\begin{align*}\int\frac{\mathrm{e}^{2x}}{\sqrt{\mathrm{e}^{x}-1}}\mathrm{d}x=&\int\frac{\mathrm{e}^{x}}{\sqrt{\mathrm{e}^{x}-1}}\mathrm{d}(\mathrm{e}^{x})=\int\frac{(\mathrm{e}^{x}-1)+1}{\sqrt{\mathrm{e}^{x}-1}}\mathrm{d}(\mathrm{e}^{x})\\=&\int\sqrt{\mathrm{e}^{x}-1}\mathrm{d}(\mathrm{e}^{x})+\int\frac{1}{\sqrt{\mathrm{e}^{x}-1}}\mathrm{d}(\mathrm{e}^{x})\\=&\frac{2}{3}(\mathrm{e}^{x}-1)\sqrt{\mathrm{e}^{x}-1}+2\sqrt{\mathrm{e}^{x}-1}+C_{1},\end{align*}$$
...
【解析】 $\lim_{x\to0}\varphi(x)=\lim_{x\to0}\frac{\int_{0}^{x}tf(t)dt}{x^{2}}$ 洛必达法则  $\lim_{x\to0}\frac{xf(x)}{2x}=\frac{1}{2}f(0)=0=\varphi(0)$，因此  $\varphi(x)$ 在 x=0 处连续.

当 $x\neq0$时，

$$\varphi^{\prime}(x)=\frac{x^{3}f(x)-2x\int_{0}^{x}tf(t)\mathrm{d}t}{x^{4}}=\frac{f(x)}{x}-\frac{2\int_{0}^{x}tf(t)\mathrm{d}t}{x^{3}};$$

当x=0时，

$$\begin{align*}\varphi^{\prime}(0)=&\lim_{x\to0}\frac{\varphi(x)-\varphi(0)}{x-0}=\lim_{x\to0}\frac{\displaystyle\int_{0}^{x}tf(t)\mathrm{d}t}{x^{3}}=\lim_{x\to0}\frac{xf(x)}{3x^{2}}\\=&\frac{1}{3}\lim_{x\to0}\frac{f(x)}{x}=\frac{1}{3}\lim_{x\to0}\frac{f(x)-f(0)}{x}=\frac{1}{3}f^{\prime}(0)\;.\end{align*}$$

$$\lim_{x\to0}\varphi^{\prime}(x)=\lim_{x\to0}\frac{f(x)}{x}-\lim_{x\to0}\frac{2\int_{0}^{x}tf(t)\mathrm{d}t}{x^{3}}=f^{\prime}(0)-\frac{2}{3}f^{\prime}(0)=\frac{1}{3}f^{\prime}(0)=\varphi^{\prime}(0)$$
...
$$\begin{align*}g(x)=&\int_{-a}^{a}\left|x-t\right|f(t)\mathrm{d}t=\int_{-a}^{x}(x-t)f(t)\mathrm{d}t+\int_{x}^{a}(t-x)f(t)\mathrm{d}t\\=&x\int_{-a}^{x}f(t)\mathrm{d}t-\int_{-a}^{x}tf(t)\mathrm{d}t+\int_{x}^{a}tf(t)\mathrm{d}t-x\int_{x}^{a}f(t)\mathrm{d}t\;.\end{align*}$$

因为 $f(x)$在 $[-a,a]$上连续，故上式中的变限积分函数均可导，求导得

$$\begin{align*}g^{\prime}(x)=&\int_{-a}^{x}f(t)\mathrm{d}t+x f(x)-x f(x)-x f(x)-\int_{x}^{a}f(t)\mathrm{d}t+x f(x)\\=&\int_{-a}^{x}f(t)\mathrm{d}t-\int_{x}^{a}f(t)\mathrm{d}t\;,\end{align*}$$
...
$$\begin{aligned}&\xrightarrow{ 令 t=-u}\int_{a}^{-a}\left|x-u\right|f(-u)\mathrm{d}(-u)\\ &\\&=\int_{-a}^{a}\left|x-u\right|f(u)\mathrm{d}u=g(x),\\ \end{aligned}$$
