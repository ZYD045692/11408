...
【分析】函数 $f(x)$表达式中含有未知参数a，可以用已知 $f(x)$在x=0和x=-1处取得极值来确定a，分析 $f'(x)$和 $f''(x)$的正负就可以定出 $f(x)$的增减区间、极大值、极小值和拐点.
...
由已知可得  $a=\frac{2}{3}$，于是  $f'(x)=2x(x+1)$， $f''(x)=4x+2$。

令 $f''(x)=0$，得 $x=-\frac{1}{2}$，列表分析：


| x | (-∞,-1) | -1 | $\left(-1,-\frac{1}{2}\right)$ | $-\frac{1}{2}$ | $\left(-\frac{1}{2},0\right)$ | 0 | (0,+∞) |
|---|---|---|---|---|---|---|---|
| f'(x) | + | 0 | - | - | - | 0 | + |
| f''(x) | - | - | - | 0 | + | + | + |
| f(x) | \uparrow | 极大值 | \downarrow | 拐点 | ↘ | 极小值 | \uparrow |
