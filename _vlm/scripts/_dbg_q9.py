...
for u in units:
    if u["unit"] == "强化篇/第9章 一元函数积分学的计算":
        nums = sorted(u["probs"], key=int)
        print("范围 pages_q:", u["pages_q"], "题数:", len(nums))
        print("定位:", {n: (p+1 if p is not None else None) for n, p in ((n, u["pp_q"].get(n)) for n in nums)})
        p0, p1 = u["pages_q"]
        for p in range(p0, p1):
            t = full.get(p, "")
            hs = re.findall(r"(?m)^\s*\d{1,2}\s*[.、．]", t)
            print(f"  p{p+1} 行首题号: {hs[:18]}")
